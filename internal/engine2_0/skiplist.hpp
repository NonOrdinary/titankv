...
public:
  ...
  SkipList() : level_(0) { head_ = new Node({}, {}, maxLevel - 1); }
  ...
  bool Search(const std::vector<uint8_t> &searchKey, std::vector<uint8_t> &val,
              uint8_t &keyType) const {
    Node *current = head_;
  ...
  void Insert(const std::vector<uint8_t> &internalKey,
  ...
  template <typename Callback> void Iterate(Callback cb) const {
