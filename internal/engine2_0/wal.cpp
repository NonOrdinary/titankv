...
bool WAL::WriteRecord(const std::vector<uint8_t> &internalKey,
  ...
  size_t offset = 4;
  PutUint32LE(write_buf_.data() + offset,
              static_cast<uint32_t>(internalKey.size()));
  offset += 4;
  ...
  if (!file_.write(reinterpret_cast<const char *>(write_buf_.data()),
                   totalSize)) {
    return false;
  }
  file_.flush();
...
uint64_t WAL::Recover(MemTable *mt) {
  ...
  while (true) {
    ...
    std::vector<uint8_t> finalIK(scratchBuf.begin(),
                                 scratchBuf.begin() + keyLen);
    ...
    if (!finalVal.empty()) {
      std::copy(finalVal.begin(), finalVal.end(),
                checkBuf.data() + 4 + finalIK.size() + 4);
    }
...
std::string WAL::GetPath() const { return path_; }
